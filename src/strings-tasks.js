...
function formatTime(minutes, seconds) {
  const formMin = String(minutes).padStart(2, '0');
...
function isPalindrome(str) {
  const newStr = str.toLocaleLowerCase().replace(/[^a-zA-Z0-9]/g, '');
...
function findLongestWord(sentence) {
  const wordsArr = sentence.replace(/[^a-zA-Z0-9 ]/g, '').split(' ');
  let longestWord = '';
...
function getCardId(value) {
  ...
  const suitOffset = suits.indexOf(suit);
  const rankOffset = ranks.indexOf(rank);

  return suitOffset * 13 + rankOffset;
}
